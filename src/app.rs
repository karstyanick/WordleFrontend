...
pub fn app() -> Html {
    ...
    let input_states = use_state(|| vec![InputState {guessed: false, word: "".to_owned()}, InputState {guessed: false, word: "".to_owned()}, InputState {guessed: false, word: "".to_owned()}, InputState {guessed: false, word: "".to_owned()}, InputState {guessed: false, word: "".to_owned()}, InputState {guessed: false, word: "".to_owned()}]);
    ...
    let input_states_for_key_down = input_states.clone();
    let current_letter_row_for_key_down = current_letter_row.clone();
    let current_letter_column_for_key_down = current_letter_column.clone();
    let word_length_for_key_down = word_length.clone();
    let chosen_word_for_key_down = chosen_word.clone();
    let keyboard_row_one_for_key_down = keyboard_row_one.clone();
    let keyboard_row_two_for_key_down = keyboard_row_two.clone();
    let keyboard_row_three_for_key_down = keyboard_row_three.clone();
    use_effect(move || {
        // Attach a keydown event listener to the document.
        let document = gloo::utils::document();
        let input_states = input_states_for_key_down.clone();
        let current_letter_row = current_letter_row_for_key_down.clone();
        let current_letter_column = current_letter_column_for_key_down.clone();
        let word_length = word_length_for_key_down.clone();
        let chosen_word = chosen_word_for_key_down.clone();
        let keyboard_row_one = keyboard_row_one_for_key_down.clone();
        let keyboard_row_two = keyboard_row_two_for_key_down.clone();
        let keyboard_row_three = keyboard_row_three_for_key_down.clone();
        
        let listener = EventListener::new(&document, "keydown", move |event|{
            let event = event.dyn_ref::<web_sys::KeyboardEvent>().unwrap_throw();
            let letter = event.key().to_uppercase();
            if(letter == "ENTER"){
                        ...
                        submitted_word.chars().enumerate().for_each(|(index, c)| {
                            if (key.letter == c){
                                if (key.guess_status == GuessStatus::NotGuessed){
                                    ...
                                    if(key.guess_status == GuessStatus::Contained && guess_status[index] == GuessStatus::Right){
                                        key = KeyboardLetter {letter: key.letter, guess_status: guess_status[index].clone()}
                                    }
                                }
                            }
            ...
            if(ALPHABET.contains(letter.as_str())){
                input_states.set(input_states.to_vec().iter().enumerate().map(|(index, input_state)| {
...
pub fn check_input_value_against_word(guessed_word: &String, word: String) -> Vec<GuessStatus>{
    let word_chars: Vec<char> = word.chars().collect();
    let mut remaining_chars: Vec<char> = word_chars.clone();

    let green_grey = guessed_word.chars().enumerate().map(|(index, letter)| {
        if(letter == word_chars[index]){
            remaining_chars.remove(remaining_chars.iter().position(|&c| c == letter).unwrap());
            return GuessStatus::Right
        }
        GuessStatus::NotGuessed
    }).collect::<Vec<GuessStatus>>();

    let yellow_grey = guessed_word.chars().enumerate().map(|(index, letter)| {
        if(remaining_chars.contains(&letter)){            
            if(green_grey[index] != GuessStatus::Right){
                remaining_chars.remove(remaining_chars.iter().position(|&c| c == letter).unwrap());
            };
            return GuessStatus::Contained
        }
        GuessStatus::NotContained
    }).collect::<Vec<GuessStatus>>();

    let guess_status = green_grey.iter().zip(yellow_grey.iter()).map(|(green_grey, yellow_grey)| {
        if(*green_grey == GuessStatus::Right){
            return GuessStatus::Right
        }else if(*yellow_grey == GuessStatus::Contained){
            return GuessStatus::Contained
        }else{
            return GuessStatus::NotContained
        }
    }).collect::<Vec<GuessStatus>>();
